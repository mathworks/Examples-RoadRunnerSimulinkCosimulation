    ...
    methods (Access = protected)
        function setupImpl(obj)
            ...
            obj.LineHandles.RefVx  = plot(obj.AxesHandles(3), ...
                0:N-1, nan(1,N), 'r--', 'LineWidth', 1.5);
            obj.LineHandles.MeaVx  = plot(obj.AxesHandles(3), ...
                0, nan, 'm*', 'MarkerSize', 10);
        ...
        function stepImpl(obj, measured, manipulated, state, reference)
            ...
            set(obj.LineHandles.MVLongAccel, ...
                'XData', tVec, ...
                'YData', ax_horizon(:)');
            ax_pred = state(:,2+obj.stateoffset);
            ...
            set(obj.LineHandles.MVSteer, ...
                'XData', tVec, ...
                'YData', steer_horizon(:)');
            if obj.stateoffset == 1
                steer_pred = state(:,1);I
            ...
            %% 3) Vx（予測 / 参照 / 測定）
            vx_pred = state(:,1+obj.stateoffset);
            vx_ref  = reference(:,1);
            vx_mea  = measured(1);
            ...
            %% 4) Y（予測 / 参照 / 測定）
            y_pred = state(:,6+obj.stateoffset);
            y_ref  = reference(:,2);
            y_mea  = measured(2);
            ...
            %% 5) Yaw (ψ)（予測 / 参照 / 測定）
            psi_pred = state(:,4+obj.stateoffset);
            psi_ref  = reference(:,3);
            psi_mea  = measured(3);
            ...
            psi_dot_pred = state(:,5+obj.stateoffset);
            ...
            vy_pred = state(:,3+obj.stateoffset);
